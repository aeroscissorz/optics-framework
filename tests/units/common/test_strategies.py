...
class TestPollFramesWithinDeadline:
    ...
    def test_sleep_is_clamped_to_remaining(self, monkeypatch):
        ...
        gen = LocatorStrategy._poll_frames_within_deadline(
            stream, end_time=100.6, poll_interval=1.5
        )

        assert list(gen) == []
        assert stream.get_all_available_screenshots.call_count == 0
